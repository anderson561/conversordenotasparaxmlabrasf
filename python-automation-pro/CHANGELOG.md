...
- **Nota nº 2086 (RADIO E TELEVISAO BANDEIRANTES DA BAHIA LTDA → PORTO UNO AGENCIA DE IMOVEIS LTDA, R$ 3.000,00), achado real 2026-09-24, a partir do PDF de origem `nota portoo.pdf` (1 página, escaneada, 180° INVERTIDA)**: é uma NFCom (tributada por ICMS), não uma NFS-e ABRASF — sem detecção dedicada, caía no fallback amplo `LAYOUT_NACIONAL` e saía com `<ValorServicos>0.00</ValorServicos>`, `<CodigoVerificacao>XXXX-XXXX</CodigoVerificacao>` (sentinela) e a razão social do tomador poluída com a chave de acesso + o texto de placeholder do QR Code PIX ("NÃO HÁ DADOS A SEREM IMPRESSOS"). **Fix:** novo layout dedicado **`nfcom_bandeirantes`**, 6º emitente do mesmo template nacional do portal SVRS já usado por `nfcom_salvador`/`nfcom_rlgr`/`nfcom_lotec_fibra`/`nfcom_sete_connect`/`nfcom_net_mund` — gate pelo CNPJ do emitente (13.810.015/0001-67, tolerante à vírgula em vez de ponto no separador do OCR) + marcador "FATURA DE SERVIÇOS DE COMUNICAÇÃO ELETRÔNICA".
  - Razão social/CNPJ do prestador (`RADIO E TELEVISAO BANDEIRANTES DA BAHIA LTDA`) sobrevivem limpos na leitura de página inteira, mas o CEP e o número do endereço saem com dígitos trocados pelo OCR ("40715-150"/"194" em vez dos reais "40215-150"/"19A", confirmados por imagem renderizada da página) — por isso o prestador é FIXO/hardcoded (mesmo racional dos 5 layouts irmãos), não extraído do OCR para esses 2 campos.
  - Tomador extraído de um bloco **SEM** rótulos "CLIENTE:"/"CNPJ:"/"ENDEREÇO:" (diferente dos 4 irmãos rotulados) — a razão social e o endereço vêm em texto corrido logo após a linha "Protocolo de Autorização:", terminando no rótulo "CNPJ/CPF:"; a linha de placeholder do QR Code PIX ("NÃO HÁ DADOS A SEREM IMPRESSOS") é descartada por filtro de conteúdo, não por posição fixa. A linha do logradouro sai colada pelo OCR ("ALSALVADOR" em vez de "AL SALVADOR", abreviação de "Alameda" sem o espaço) — corrigida por um regex que reinsere o espaço só quando a linha começa literalmente por "AL" seguido de maiúscula.
  - Competência: a "REFERÊNCIA" sai com o ano corrompido nesta nota ("08/1016" em vez de "08/2026") — em vez de arriscar um ano errado, usa o mês/ano da Data de Emissão já resolvida (mesmo racional já usado em `LAYOUT_GUARULHOS` para um campo redundante ilegível).
  - A caixa "TOTAL A PAGAR: R$ 3.000,00" se funde com a linha "DATA DE EMISSÃO" na leitura de página inteira e o PRÓPRIO VALOR sai corrompido junto do rótulo ("EURAL – PAGAR: R$ 3.099,09" em vez do real R$ 3.000,00) — recuperada por um recorte dedicado (`_ocr_recut_total_pagar_nfcom_bandeirantes`, 49%-100% da largura, 14,8%-17,0% da altura, MESMO zoom 3x da leitura padrão — zooms maiores degradam o separador decimal), rotacionado pelo ângulo já detectado por `_ocr_page` (nota escaneada e 180° invertida) e prependado ao texto principal.
  - Código de Verificação = chave de acesso de 44 dígitos ("2926 0813 8100 1500 0167 6210 0000 0020 8610 6115 0820") — sobrevive limpa e completa na leitura de página inteira desta nota (raro entre os 6 irmãos), validada pelo dígito verificador mod-11 (`_dv_chave_nfe`) antes de aceitar; fallback honesto `'NFCOM'` quando o dígito verificador não bater. Decodificação estrutural bate exatamente contra os demais campos confirmados (cUF=29 Bahia, AAMM=2608, CNPJ do emitente nas posições 6:20, modelo=62, série=100, número=000002086).
  - Base de Cálculo/Alíquota/ISS mantidos em 0,00 propositalmente (ICMS, não ISS), com aviso. Salvador/BA já estava em `KNOWN_CITIES` (IBGE `2927408`) — nenhuma cidade nova cadastrada.
  - Conversão real conferida contra o PDF de origem: `ValorServicos=3000.00`, prestador/tomador com razão social e endereço corretos e limpos, `CodigoVerificacao=29260813810015000167621000000020861061150820` (chave válida, não mais sentinela).
  - Suíte: 804 → **815 testes** (11 novos em `tests/test_nfcom_bandeirantes_layout_novo.py`, usando o texto REAL de `_extract_via_ocr` do PDF de origem como fixture, não um mock sintético).
  - Contagem de layouts: **63 → 64** (nova constante `LAYOUT_NFCOM_BANDEIRANTES`).
...
- **Nota nº 171/173 (SUL&SEG COMERCIO E SERVICOS DE MANUTENCAO ELETRICOS LTDA → MACEDO COMERCIAL DE CALCADOS LTDA, Lauro de Freitas/BA, R$ 40,00), DANFSe v2.0 (`LAYOUT_NACIONAL_REFORMA`), a partir do PDF real `Nota Macedo 173.pdf`: XML saía com `Prestador Não Identificado`, `Tomador Não Identificado`, `ValorServicos=0.00`, `CodigoVerificacao=XXXX-XXXX` (sentinela) e `DataEmissao` no fallback `datetime.now()` — apesar de a nota ser 100% DIGITAL (nunca escaneada).**
  - **Causa-raiz**: `pdfminer.high_level.extract_text()` extrai o texto embutido desta nota especificamente SEM NENHUM ESPAÇO entre palavras/rótulos/valores (glifos posicionados sem gap detectável pelo layout do pdfminer) — confirmado não ser um parâmetro de tuning: `LAParams(word_margin=...)` testado de 0.1 a 2.0 devolve resultado idêntico (zero espaços sempre) — é como este gerador de PDF específico codifica o texto. Isso derruba toda extração de `LAYOUT_NACIONAL_REFORMA` baseada em `\s+` entre rótulo e valor, mesmo a DETECÇÃO de layout continuando a funcionar por acidente (usa `\s*`, que também casa zero espaços) — por isso o sintoma nunca foi "layout não reconhecido", e sim entidades/valores todos vazios/zerados sob o layout certo.
  - **Fix — fallback para OCR por patologia de extração digital, não remendo de regex para texto colado**: novo `SPPdfExtractor._texto_digital_sem_espacos` detecta a patologia de forma GENÉRICA (razão espaços/tamanho do texto, texto longo o bastante para ser conclusivo) — não depende do CNPJ/emissor desta nota, cobrindo qualquer outra nota do mesmo gerador de PDF no futuro. Em `parse_multiple()`, quando uma página digital cai nessa patologia E já é detectada como `LAYOUT_NACIONAL`/`LAYOUT_NACIONAL_REFORMA` (layouts cuja extração depende inteiramente de espaço entre rótulo e valor), o texto digital é descartado e a página é reprocessada via `_ocr_page()` — reaproveitando o pipeline de OCR já validado, em vez de reescrever cada regex do layout para tolerar texto colado (frágil e arriscado). Restrito a esses dois layouts para não desviar para OCR, sem necessidade, outros layouts do projeto cuja extração já tolera texto reordenado/colado por outros meios (ex.: `_reconstruir_texto_por_coordenadas` para Camaçari/Goiânia).
  - **`_extrair_entidade_nacional_reforma` estendida para o formato de grade OCR desta nota** (a extração já era compatível com "rótulo numa linha, valor na seguinte" — desenhada originalmente para uma nota escaneada — mas esta nota expôs 3 variações novas de colunas fundidas):
    1. Quando duas colunas fundidas no cabeçalho ("Nome / Nome Empresarial Município / Sigla UF") têm sua linha de VALOR também fundida ("`<razão social>` Lauro de Freitas / BA") e o Código IBGE/CEP do PRESTADOR vem numa seção SEPARADA (sem IBGE colado na própria linha fundida, ao contrário da nota nº 5/SBS já coberta) — o fallback de corte por prova de IBGE agora também aceita o `cod_mun` já lido separadamente como a mesma prova, não só um IBGE presente na própria linha fundida. Sem isso, a razão social engolia o nome do município inteiro ("...LTDA Lauro de Freitas") e o Município ficava vazio.
    2. Indicador Municipal (Inscrição) e Telefone fundidos numa única linha de cabeçalho ("Indicador Municipal (Inscrição) Telefone") com o valor também numa única linha ("10030574 -", ou "04.074.648/0003-25 - (71) 3342-4542" quando o CNPJ também está na mesma linha) — os dois aceitadores passam a extrair o token/substring relevante da linha, em vez de exigir a linha inteira.
    3. E-mail do tomador sem linha de valor própria (a coluna "E-mail" nunca imprime nada abaixo dela) — o e-mail só existe colado no FIM da linha de Endereço ("...ITINGA valmirGQmaxcalcados.com.br"), já removido do endereço por um fix anterior (nota nº 5/SBS) mas descartado em vez de aproveitado como e-mail. Recuperado do mesmo fragmento removido.
  - **`_corrigir_arroba_ocr` generalizada de 1 para 1+ maiúsculas soltas representando o "@"** ("valmirGQmaxcalcados.com.br" → "valmir@maxcalcados.com.br"; o achado original, BioControl nº 36345, só tinha 1 maiúscula solta).
  - **Item da LC 116 saindo no default genérico "03115" (locação de bens móveis) em vez do real "1102" (vigilância/segurança), achado ao conferir a conversão REAL contra o PDF de origem** — `_extrair_codigo_servico` (`LAYOUT_NACIONAL_REFORMA`) ancorava só em "SERVIÇO PRESTADO" seguido do código, ordem válida para as notas nº 11/59 já cobertas; nesta nota a ordem é INVERTIDA — o rótulo "Código de Tributação Nacional/Municipal" vem ANTES de "SERVIÇO PRESTADO", com o código colado logo depois do próprio rótulo, na mesma linha fundida com "Código da NBS". **Fix:** usa qualquer uma das duas âncoras que aparecer primeiro no texto, generalizando sem regredir os dois casos já cobertos (lá, "SERVIÇO PRESTADO" sempre vem primeiro; aqui, o rótulo).
  - Conferido também nesta nota, sem necessidade de correção: Número (171), CNPJ do tomador e demais campos de valores (BC ISSQN, Alíquota, ISSQN Apurado, Retenção do ISSQN) já saíam corretos mesmo sob a patologia — o parser de `LAYOUT_NACIONAL_REFORMA` já tolerava o texto colado o bastante para esses campos específicos.
  - **Achado colateral, não corrigido (fora do escopo desta correção)**: `Competencia` sai como a DATA COMPLETA impressa em "Competência da NFS-e" (`2026-09-08`), não truncada para o 1º dia do mês (`2026-09-01`) — comportamento do branch `LAYOUT_NACIONAL`/`LAYOUT_NACIONAL_REFORMA` de `_extrair_competencia`, não tocado por este fix (só trunca quando o rótulo imprime `MM/AAAA`; quando imprime a data completa, usa-a integralmente, mesmo tratamento já validado nas notas nº 11/59/5-SBS). Sinalizado aqui porque diverge da expectativa inicial de truncar sempre para o 1º dia do mês; decisão de negócio pendente de confirmação do usuário antes de alterar um comportamento compartilhado por várias notas já em produção.
  - **Achado colateral, não corrigido (fora do escopo)**: o mesmo CNPJ do prestador (`18294792000110`, SUL&SEG) já existe hardcoded em `_extrair_entidade` para `LAYOUT_SULSEG_COBRANCA` (fatura de cobrança da mesma empresa), com endereço "AV. BRIGADEIRO ALBERTO COSTA MATOS, 103, Aracuí" — o número não bate com o desta nota ("1184") e "CENTRO"/"ARACUI" aqui podem ser complemento+bairro, não só bairro. Extraído fielmente como impresso NESTA nota, sem tentar reconciliar com o outro layout — pode ser a mesma empresa com endereços de cobrança/prestação diferentes, ou um erro de OCR/digitação num dos dois documentos; investigação futura, se necessário.
  - Suíte: 815 → **820 testes** (5 novos em `tests/test_danfse_nacional_reforma_texto_digital_sem_espacos.py`, usando o texto digital real via `extract_text()` — a patologia sem espaço nenhum — e o texto OCR real via `_extract_via_ocr()`, ambos do PDF de origem, como fixtures). Conversão real (`app.py --input`) conferida contra o PDF de origem após o fix.
  - Contagem de layouts inalterada em **64** — correção em layout já existente (`LAYOUT_NACIONAL_REFORMA`), não um layout novo.
...
- **Nota nº 683 (NET MUND BRASIL INTERNET E SEGURANCA LTDA → RG RESTAURANTE LTDA - EPP, R$ 150,00), achado real 2026-09-23, a partir do PDF de origem `RG RESTAURANTE - NFSe TERCEIROS.pdf`**: é uma NFCom (tributada por ICMS), não uma NFS-e ABRASF — sem detecção dedicada, caía no fallback amplo `LAYOUT_NACIONAL` e saía com o prestador sentinela (`Prestador Não Identificado`, CNPJ `00000000000100`) e o `<Numero>` do endereço do tomador poluído com um blob de ~250 caracteres vazado de outras seções da nota (data de emissão, chave de acesso, protocolo, Cod. Assinante/Contrato). **Fix:** novo layout dedicado **`nfcom_net_mund`**, 5º emitente do mesmo template nacional do portal SVRS já usado por `nfcom_salvador`/`nfcom_rlgr`/`nfcom_lotec_fibra`/`nfcom_sete_connect`.
  - **Detecção ATÍPICA em relação aos 4 irmãos**: nesta nota específica, TODA a faixa cinza do cabeçalho (título + CNPJ impresso do prestador) falha por completo na leitura de página inteira do Tesseract — nem o CNPJ impresso nem a marca "FATURA DE SERVIÇOS..." sobrevivem ao OCR. O gate usa a decodificação estrutural da própria chave de acesso de 44 dígitos (dígitos `[6:20]` = CNPJ do emitente `37976210000120`, mesma técnica já validada no `nfcom_lotec_fibra` para o cUF) combinada com a URL do portal nacional do QR Code (`dfe-portal.svrs.rs.gov.br/Nfcom`), que sobrevive fora da faixa cinza.
  - Razão social do prestador (`NET MUND BRASIL INTERNET E SEGURANCA LTDA` — sem "O" final em "MUND" e sem cedilha em "SEGURANCA", impresso assim mesmo no documento) confirmada em 2 fontes independentes: imagem renderizada da página e consulta pública do CNPJ na Receita Federal.
  - A caixa "TOTAL A PAGAR: R$ 150,00" também some da leitura de página inteira (mesma faixa cinza), apesar de nitidamente legível na imagem da página — recuperada por um recorte dedicado (`_ocr_recut_total_pagar_nfcom_net_mund`, zoom 6x + `--psm 6`), prependado ao texto principal.
  - Tomador extraído dinamicamente do bloco "CLIENTE:"/"CNPJ:"/"ENDEREÇO:" (mesmo formato dos irmãos) — o parser dedicado captura só as 2 linhas do bloco de endereço, o que já elimina o vazamento por construção. Achado adicional nesta nota: o endereço do tomador tem um hífen SEM espaços dentro de um nome composto (`BOULEVARD-GIRAFFAS`, complemento) — o separador de campo do regex exige espaço nos 2 lados (`\s+-\s+`) para não quebrar esse nome ao meio, ao contrário do `\s*-\s*` (espaço opcional) usado pelos irmãos.
  - Competência via "REFERÊNCIA (ANO/MÊS)"; Data de Emissão via "DATA DE EMISSÃO...às..." (ambas sobrevivem normalmente nesta nota); Código de Serviço fixo "0000"; Código de Verificação = chave de acesso de 44 dígitos com fallback honesto `'NFCOM'`; Base de Cálculo/Alíquota/ISS mantidos em 0,00 propositalmente (ICMS, não ISS), com aviso.
  - Camaçari/BA (prestador e tomador) já estava em `KNOWN_CITIES` (IBGE `2905701`) — nenhuma cidade nova cadastrada.
  - Suíte: 793 → **804 testes** (11 novos em `tests/test_nfcom_net_mund_layout_novo.py`, usando o texto REAL de `_extract_via_ocr` do PDF de origem como fixture, não um mock sintético).
  - Contagem de layouts: **62 → 63** (nova constante `LAYOUT_NFCOM_NET_MUND`).
