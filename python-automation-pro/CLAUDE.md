...
> **ORIGEM:** Este arquivo é a versão adaptada, para o formato nativo do Claude Code, do ecossistema de agentes originalmente definido em `.windsurfrules` / `.agents/AGENTS.md` / `.agent/`. O `.windsurfrules` e o `.agents/AGENTS.md` permanecem neste repositório para compatibilidade com outras ferramentas (Windsurf etc.), mas o `.agent/` (`rules/`, `skills/`, `workflows/`, `specs/`) foi removido do repositório: o usuário migrou esse conteúdo para um escopo **global** em `~/.claude/agents/`, para reutilizar os mesmos agentes/skills/rules/workflows/specs em qualquer projeto sem precisar duplicá-los por repositório. O Claude Code lê **este** `CLAUDE.md` automaticamente em toda conversa neste repositório, e carrega o conteúdo global de `~/.claude/agents/` em qualquer sessão, independentemente do projeto.
> **Última atualização:** 2026-08-27 — `.agent/` removido do controle de versão deste repositório (migrado para `~/.claude/agents/`, escopo global). Atualização anterior: 2026-08-26 — registradas 10 skills novas encontradas em `.claude/skills/` (Seção 0 e matriz de delegação da Seção 3), que ainda não constavam neste arquivo. Atualização anterior: 2026-07-10 — conversão completa do ecossistema `.agent/` para `.claude/agents/`, `.claude/skills/`, `.claude/commands/` e `.claude/specs/`.
...
`.windsurfrules` e `.agents/AGENTS.md` permanecem no repositório inalterados, para uso por outras ferramentas (Windsurf, etc.). O `.agent/rules/`, `.agent/skills/`, `.agent/workflows/` e `.agent/specs/` que existiam aqui **foram removidos deste repositório** (2026-08-27) — o usuário migrou esse conteúdo para escopo **global**, em `~/.claude/agents/{rules,skills,workflows,specs}/`, reutilizável em qualquer projeto sem duplicação por repositório. Este `CLAUDE.md` e a pasta `.claude/` (deste projeto) continuam sendo a tradução funcionalmente equivalente para o Claude Code, específica do `python-automation-pro`; o conteúdo global em `~/.claude/agents/` é carregado independentemente do projeto e pode conter agentes/skills adicionais (ex.: Django Architect, MVC Architect) sem relação com este repositório.
